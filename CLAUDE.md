...
- **Input**: WASD move (camera-relative), Shift sprint, Left-click attack/interact/open dialogue (click NPC/entity), E interact, I inventory, C status, K skills, P proficiencies, J quest log, M world map, N minimap, 1-5 hotbar, Esc close/settings
...
- **DialoguePanel** (`scenes/ui/dialogue_panel.gd`): Bottom-screen JRPG-style dialogue box. Full width, anchored to bottom. NPC name + text in the box. Choices float as VBoxContainer on right above the box. NPC portrait floats above the box on the left (loaded from `assets/textures/ui/portraits/{npc_id}.png`, hidden if no file exists). Trade action keeps dialogue open with "Buy (Xg)" + "Close Shop" choices; parting words on shop close. Hides HUD/minimap/panel toggles when open. Signals: `trade_requested(npc_node)`. Quest actions: `quest_accept:id`, `quest_complete:id`. Quest actions: `"quest_accept:id"`, `"quest_complete:id"`. Trade action keeps dialogue open with "Buy (Xg)" + "Close Shop" choices; buy button updates dynamically via `cart_changed` signal; parting words shown on shop close. Click NPC body → opens DialoguePanel. Escape closes.
...
- **InteriorManager** (`scripts/world/interior_manager.gd`): In-zone instantiation at Y=-50. Loading screen transitions. Camera yaw 45° for interiors. Hides exterior (zone NavigationRegion3D + NPCs). Switches nav map. Enter via DoorTrigger click, exit via walk-through Area3D or Escape
...
- **DialoguePanel** (`scenes/ui/dialogue_panel.gd`): Bottom-screen JRPG-style dialogue box. Full width, anchored to bottom. NPC name + text in the box. Choices float as VBoxContainer on right above the box. NPC portrait floats above the box on the left (loaded from `assets/textures/ui/portraits/{npc_id}.png`, hidden if no file exists). Trade action keeps dialogue open with "Buy (Xg)" + "Close Shop" choices; parting words on shop close. Hides HUD/minimap/panel toggles when open. Signals: `trade_requested(npc_node)`. Quest actions: `quest_accept:id`, `quest_complete:id`.
...
- **StatusPanel** (`scenes/ui/status_panel.gd`): Character stats panel (C key) — 2-column layout: Offensive (ATK/MATK/CritRate/CritDmg) + Defensive (HP/DEF/MDEF) left, Speed (AtkSpd/MoveSpd/CastSpd) + Resource (Stamina/HPRegen/CDR) right. Proficiency icons in 2-column grid below.

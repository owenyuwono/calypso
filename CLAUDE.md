...
- **Fonts**: Marcellus-Regular for body text (`GAME_FONT`, project default). Philosopher-Bold for display/titles (`GAME_FONT_DISPLAY`). Set in `project.godot` + `UIHelper` constants
- **Autoload singletons**: WorldState (entity registry), GameEvents (signals), TimeManager (24h day cycle), ZoneManager (zone transitions), AudioManager (UI SFX pool + master mute on startup), ResourceManager (container-based resource simulation)
- **Static utility classes**: ModelHelper (3D models, effects, animation merging, `create_toon_material()`, `apply_toon_to_model()`, `create_fallback_mesh()`), UIHelper (panel styles, UI helpers), EntityHelpers (entity lifecycle utilities), TerrainGenerator (terrain mesh generation), HitVfx (combat hit effects), BuildingHelper (building + prop creation), AssetSpawner (model spawning, material caching), BiomeScatter (exclusion zones, scatter algorithm)- **Composition nodes**: EntityVisuals (visual state: model, overlay, animations) + AudioComponent (per-entity positional audio) + AutoAttackComponent (signal-based auto-attack) + PerceptionComponent (Area3D-based spatial awareness) + entity components (stats, inventory, equipment, combat, stamina) for all entities
...
- `StatsComponent` — hp, max_hp, atk, def, attack_speed, attack_speed_mult, attack_range, move_speed, max_stamina, stamina_regen, hp_regen, level. **Must set `.name = "StatsComponent"` before `add_child()`**. API: `take_damage()`, `heal()`, `restore_full_hp()`, `is_alive()`. Syncs to `WorldState.entity_data` via `_sync()`.
...
- `StaminaComponent` — stamina pool with fatigue system. No passive drains — only depletes from explicit `drain_flat()` calls and blocking. Regen rates: 1.0/sec out-of-combat, 3.0/sec rest spots, 0.3/sec in-combat (all × stamina_regen from StatsComponent). Fatigue: low stamina softly debuffs ATK (0.9× at 0%), Move Speed (0.8×), Attack Speed (0.85×). API: `get_stamina()`, `get_stamina_percent()`, `get_max_stamina()`, `drain_flat()`, `get_fatigue_multiplier(stat_type)`. Signal: `stamina_changed` (emitted via GameEvents on 10% threshold crossings).
- `EntityVisuals` — visual state composition node. `setup_model(path, scale, color)` for single-file models. `setup_model_with_anims(mesh_path, anim_paths, scale, color)` for Meshy characters with separate animation files (auto-shifts center-origin models). `setup_custom_model(model, mesh_instances)` for procedural meshes. API: `play_anim()`, `crossfade_anim()`, `face_direction()`, `flash_hit()`, `highlight()`, `unhighlight()`, `set_state_tint()`, `fade_out()`, `flash_target()`, `update_weapon_visual()`, `get_anim_player()`.
...
- **StatusPanel** (`scenes/ui/status_panel.gd`): GameMenu Status tab — stat display (ATK, DEF, HP, Atk Speed, Move Speed, Stamina, HP Regen).
...
## Item System
- **ItemDatabase** (`scripts/data/item_database.gd`): 3 placeholder items. Weapons: `fists` (blunt, atk 0, speed 1.5), `wooden_plank` (blunt, atk +4, speed 0.9). Consumable: `bandage` (heal 20). Item fields: `name`, `type`, `slot_type`, `weapon_type`, `phys_type`, `atk_bonus`, `attack_speed`, `heal`, `value`. Also holds `RESISTANCE_MULTIPLIERS` and `ARMOR_PHYS_TYPE_TABLE` constants for the damage pipeline. API: `get_item(id)`.
...
- **ResourceManager** (`autoloads/resource_manager.gd`): Global autoload. Container-based resources (electricity in batteries, water in tanks, fuel in jerry cans/drums). Real-time simulation in `_process(delta)`. Rates defined per game-hour, applied continuously (1 game-hour = 112.5 real seconds). Cascade shutdown when resources deplete. Grid connection (pre-apocalypse: unlimited electricity/water for gold). API: `add_container()`, `remove_container()`, `add_device()`, `remove_device()`, `set_device_active()`, `try_consume()`, `get_resource_total()`, `get_resource_capacity()`, `trigger_grid_outage()`, `fail_grid_permanently()`.
- **ResContainer** (inner RefCounted class): Physical storage — `id`, `type`, `resource_type`, `current`, `capacity`. Methods: `add()`, `consume()`, `is_empty()`, `to_dict()`, `from_dict()`.
- **Device** (inner RefCounted class): Producer/consumer — `id`, `type`, `produces`, `consumes`, `active` (player toggle), `shutdown` (cascade). `is_running()` = `active AND NOT shutdown`. Time-of-day modifiers for solar panels.
- **DeviceDatabase** (`scripts/data/device_database.gd`): Static device definitions. Categories: producer (generators, pumps, collectors), farm (soil, vertical, aquaponics), storage (batteries, tanks, cans), utility (fridge).
- **NeedsComponent** (`scripts/components/needs_component.gd`): Player personal needs (hunger, thirst, hygiene, health, 0-100). Real-time decay. Health is consequence meter (damaged by other needs at 0). Threshold debuffs: hunger 0 = ATK 0.5x, thirst 0 = speed 0.7x, hygiene <25 = disease risk. API: `eat()`, `drink()`, `restore_hygiene()`, `heal_health()`, `get_hunger_atk_multiplier()`, `get_thirst_speed_multiplier()`.
- **GameEvents signals**: `resources_updated`, `resource_depleted`, `device_shutdown`, `grid_status_changed`, `needs_changed`, `need_critical`.

## Planned Systems (Not Yet Implemented)
- Base building placement UI (grid-based)
- Apocalypse progression system (5 stages: crisis → war → inflation → virus → zombies)
- Scavenging/looting system
- Defense systems (fortifications against zombie waves)

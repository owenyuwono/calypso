...
- **Input**: Left-click move/attack/interact, E interact, I inventory, C status, S skills, P proficiencies, V vend setup, 1-5 hotbar, Esc settings
...
- **NPC scaling**: `NpcGenerator` creates 25 procedural NPCs (5 archetypes: warrior/mage/rogue/ranger/merchant, ~200 name pool, tiered loadouts, random trait_profile selection from archetype's allowed profiles). `main.gd` spawns them at world init

...
func _rebuild(skill_id: String) -> void:
	...
	var style: StyleBoxFlat = UIHelper.create_style_box(Color(0.1, 0.09, 0.07, 0.95), Color(0.45, 0.38, 0.25, 0.8), 4, 1)
...
func _build_header(skill_id: String, skill: Dictionary, skill_level: int, eff_percent: int) -> void:
	...
	# Skill icon 24x24
	var icon_path: String = _load_skill_icon_path(skill_id)
	var icon: TextureRect = UIHelper.create_icon(icon_path, Vector2(24, 24), CanvasItem.TEXTURE_FILTER_LINEAR)
	if icon:
		row.add_child(icon)

	var name_label: Label = UIHelper.create_label(skill.get("name", skill_id), 14, _effectiveness_color(eff_percent))
	name_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	name_label.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	row.add_child(name_label)

	var eff_label: Label = UIHelper.create_label("%d%%" % eff_percent, 13, _effectiveness_color(eff_percent))
	...
	if not desc.is_empty():
		var desc_label: Label = UIHelper.create_label(desc, 11, Color(0.6, 0.6, 0.6))
...
func _build_primary_proficiency(_skill_id: String, primary: Dictionary) -> void:
	...
	var label: Label = UIHelper.create_label("%s Lv. %d/%d" % [display_name, current_level, req_level], 11, Color(0.3, 0.8, 0.3) if met else Color(0.8, 0.4, 0.3))
	label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(label)

	var marker: Label = UIHelper.create_label("+" if met else "x", 11, Color(0.3, 0.9, 0.3) if met else Color(0.9, 0.3, 0.3))
...
func _build_secondary_proficiencies(skill_id: String) -> void:
	...
	for entry in secondaries:
		...
		var lbl: Label = UIHelper.create_label("%s: %s +%.1f%%" % [display_name, bonus_label_text, computed_value], 10, Color(0.5, 0.7, 0.5))
		row.add_child(lbl)


func _build_danger_warning(eff_percent: int) -> void:
	var warn_label: Label = UIHelper.create_label("! Risk of self-harm (%d%%)" % eff_percent, 11, Color(1.0, 0.4, 0.4))
...
func _add_grid_row(grid: GridContainer, label_text: String, value_text: String, value_color: Color) -> void:
	grid.add_child(UIHelper.create_label(label_text, 11, Color(0.5, 0.45, 0.38)))
	grid.add_child(UIHelper.create_label(value_text, 11, value_color))


func _load_skill_icon_path(skill_id: String) -> String:
	...
	return "res://assets/textures/ui/skills/bases/" + category + "_base.png"

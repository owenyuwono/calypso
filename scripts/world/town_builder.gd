...
static func build_walls(ctx: WorldBuilderContext) -> void:
	...
	# East gate towers — placed outside the gap so they don't block passage
	_build_tower(ctx, Vector3(70, 0, -7), 6.0, 3.0, wall_mat)
	_build_tower(ctx, Vector3(70, 0, 7), 6.0, 3.0, wall_mat)

	# East gatehouse archway — visual only (no collision), placed above walking height
	...
	# West gatehouse archway — visual only (no collision), placed above walking height
	var west_arch := MeshInstance3D.new()
	var west_arch_mesh := BoxMesh.new()
	west_arch_mesh.size = Vector3(wall_thickness, 1.5, 10.0)
...
static func place_props(ctx: WorldBuilderContext) -> void:
	...
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-15, 0, -15), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(15, 0, -15), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-15, 0, 15), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(15, 0, 15), 0.0, 0.5)

	# Street lamps — main east-west road (z≈0) every ~20 units from x:-50 to x:50
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-50, 0, 0), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-30, 0, 0), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-10, 0, 0), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(10, 0, 0), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(30, 0, 0), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(50, 0, 0), 0.0, 0.5)

	# Street lamps — market district entrances (x≈-42..-58, z≈18..22)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-42, 0, 15), 0.0, 0.5)
	AssetSpawner.spawn_prop(ctx, "street_lamp.glb", Vector3(-58, 0, 15), 0.0, 0.5)

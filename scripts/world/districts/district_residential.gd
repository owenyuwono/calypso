...
static func build(ctx: WorldBuilderContext) -> void:
	...
	_build_houses(ctx, nav_region, noise, hs)
	_build_cluster_b(ctx, nav_region, noise, hs)
	_build_inn(ctx, nav_region, noise, hs)
	_build_well(ctx, nav_region, noise, hs)
	_build_new_houses(ctx, nav_region, noise, hs)


static func _build_houses(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
...
static func _build_cluster_b(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	# B1 Boarding House
	BuildingHelper.create_building(ctx, nav_region,
...
static func _build_inn(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	...
	var inn_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.52, 0.4, 0.28))
	...
	var inn_roof_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.4, 0.18, 0.1))
	inn_roof.set_surface_override_material(0, inn_roof_mat)
	inn_roof.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	nav_region.add_child(inn_roof)


static func _build_new_houses(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
...
static func _build_well(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	var well := Node3D.new()
	well.position = Vector3(-42, BuildingHelper.snap_y(noise, -42, -35, hs), -35)

	var stone_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.5, 0.47, 0.42))
	var wood_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.4, 0.3, 0.2))

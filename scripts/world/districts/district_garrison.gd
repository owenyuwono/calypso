...
static func build(ctx: WorldBuilderContext) -> void:
	...
	_build_barracks(ctx, nav_region, noise, hs)
	_build_training_dummies(ctx, nav_region, noise, hs)
	_build_weapon_racks(ctx, nav_region, noise, hs)
	_build_archery_targets(ctx, nav_region, noise, hs)
	_build_cluster_e(ctx, nav_region, noise, hs)
	_build_new_buildings(ctx, nav_region, noise, hs)
	_build_new_props(ctx, nav_region, noise, hs)


static func _build_barracks(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
...
static func _build_training_dummies(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	var dummy_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.45, 0.35, 0.22))
	var head_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.6, 0.5, 0.35))
...
static func _build_weapon_racks(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	var rack_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.4, 0.3, 0.2))
...
static func _build_archery_targets(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	var rack_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.4, 0.3, 0.2))
	var target_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.8, 0.3, 0.2))
	...
	for tpos: Vector3 in target_positions:
		...
		target_post.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
		...
		target_face.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
		nav_region.add_child(target_face)


static func _build_cluster_e(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	# Guard Tower
	BuildingHelper.create_building(ctx, nav_region,
...
static func _build_new_props(ctx: WorldBuilderContext, nav_region: Node, noise: FastNoiseLite, hs: float) -> void:
	var flag_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.6, 0.15, 0.15))
	var pole_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.5, 0.42, 0.32))
	...
	# Torch material
	var torch_body_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(0.35, 0.25, 0.15))
	var flame_mat: StandardMaterial3D = AssetSpawner.get_or_create_color_mat(ctx, Color(1.0, 0.6, 0.1))
	...
	for tx: float in [58.5, 61.5]:
		...
		t_body.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
